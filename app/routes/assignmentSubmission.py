...
@router.get("/program/{program_id}", response_model=List[ModuleResponse])
def get_modules_by_program(program_id: UUID, db: Session = Depends(get_db)):
    modules = db.query(ModuleResponse).filter(ModuleResponse.program_id == program_id).all()
    if not modules:
        raise HTTPException(status_code=404, detail="No modules found for this program")
    return modules
